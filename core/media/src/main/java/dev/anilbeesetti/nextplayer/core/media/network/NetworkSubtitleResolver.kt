 ...
 *
 * A share that stores `Movie.mkv` next to `Movie.srt` or `Movie.vi.srt` is common, and the player
 * would otherwise show no subtitles for it at all. Lookups run over the session that already plays the
 * video, so they reuse its connected client instead of opening a second connection to the same server.
 *
...
class NetworkSubtitleResolver @Inject constructor(
     ...
     *
     * A file counts as a sibling when it sits in the video's own folder, is a regular file, and
     * matches the video according to [SubtitleSiblings.match], including language suffixes like
     * `Movie.vi.srt`.
     *
    ...
    suspend fun findAdjacentSubtitles(videoUri: Uri): List<Uri> = runCatching {
        sessions.withTarget(videoUri) { connection, client, videoPath ->
            ...
            client.listFiles(folder).getOrThrow()
                ...
                .filter { file -> SubtitleSiblings.match(videoFileName, file.name) != null }
